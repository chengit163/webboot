$(function () {
    ...
    $('#system_menu_edit_form').bootstrapValidator({
        feedbackIcons: {
...
function systemMenuEdit() {
    $('#system_menu_edit_form').data('bootstrapValidator').validate();
    if ($('#system_menu_edit_form').data('bootstrapValidator').isValid()) {
        systemMenuDialog.modalDialogShowLoading();
        $.ajax({
            type: 'PUT',
            url: '/system/menu',
            data: $('#system_menu_edit_form').serialize(),
            success: function (json) {
                if (json.code == 0) {
                    var row = $('#system_menu_edit_form').serializeJson();
                    var index = systemMenuTable.find('tr[data-uniqueid="' + row['id'] + '"]').attr('data-index');
                    if (/^\d+$/.test(index)) {
                        delete row['id'];
                        delete row['pid'];
                        systemMenuTable.bootstrapTable('updateRow', {index: index, row: row});
                    }
